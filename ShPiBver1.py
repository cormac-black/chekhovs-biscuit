...
from gpiozero import Button # relic, remove
...
#XXXXXXXXXXXXX USER INPUT XXXXXXXXXXXXXXXXXXX
print_photo = False #to print physical photo  
...
def add_frame(background, choice): #takes in openCV image (of picture taken)
	...
	if 'Expression' in choice:
		foreground = cv2.imread("Pink_Comic_CFE_Final_2.png", cv2.IMREAD_UNCHANGED) 
	elif 'Scientific' in choice:
		foreground = cv2.imread("Blue_Comic_CSD_Final.png", cv2.IMREAD_UNCHANGED)
	elif 'Justic' in choice:
		foreground = cv2.imread("Green_Comic_CSJ_Final_2.png", cv2.IMREAD_UNCHANGED) 
...
def send_email(processed_img_path):
	...
	subject = 'Hello from PCAS! (No Reply)'
	html_content = f"""
	<html>
	  <body>
	    <p> Hello Superhero,
	    ...
	    <br>
	    Please email pcas.adving@umich.edu with questions
...
def on_button_pressed(win, student_info):
	...
	spool_text(win, prompt, 2, 2, 0.07)  # New text display call
...
def main(stdscr):
	...
	prompt9 = "PCAS web and program visibility?"
